...
class ClientesReport(Report):

    def __init__(self, *args, **kargs):
        super(ClientesReport, self).__init__(*args, **kargs)
        self.title = 'Relatorio de Clientes'
...
class DadosCliente(ReportBand):
    ...
    def inserir_informacoes_pf(self, cpf, rg):
        if cpf:
            txt = ObjectValue(attribute_name='pessoa_fis_info.format_cpf',
                              top=0.3 * cm, left=8.1 * cm, width=4 * cm, height=0.5 * cm)
            txt.style = {'fontName': REPORT_FONT_BOLD,
                         'fontSize': 10, 'leading': 10}
            self.elements.append(txt)
        ...
        if rg:
            txt = ObjectValue(attribute_name='pessoa_fis_info.format_rg',
                              top=0.3 * cm, left=13 * cm, width=6.4 * cm, height=0.5 * cm)
            txt.style = {'fontName': REPORT_FONT_BOLD,
                         'fontSize': 10, 'leading': 10}
            self.elements.append(txt)
    ...
    def inserir_informacoes_endereco(self, endereco, municipio, uf, cep):
        ...
        if uf:
            txt = ObjectValue(attribute_name='endereco_padrao.uf', display_format='UF: %s',
                              top=1.6 * cm, left=8.1 * cm, width=4 * cm, height=0.5 * cm)
            txt.style = {'fontName': REPORT_FONT, 'fontSize': 10, 'leading': 10}
            self.elements.append(txt)

        if cep:
            txt = ObjectValue(attribute_name='endereco_padrao.cep', display_format='CEP: %s',
                              top=1.6 * cm, left=13 * cm, width=19.4 * cm, height=0.5 * cm)
            txt.style = {'fontName': REPORT_FONT, 'fontSize': 10, 'leading': 10}
            self.elements.append(txt)

    def inserir_informacoes_telefone(self, telefone):
        ...
        if telefone:
            ...
            txt = ObjectValue(attribute_name='telefone_padrao.telefone',
                              display_format='Tel: %s', top=top, left=0.3 * cm, width=8 * cm, height=0.5 * cm)
            txt.style = {'fontName': REPORT_FONT, 'fontSize': 10, 'leading': 10}
            self.elements.append(txt)

    def inserir_informacoes_email(self, email):
        if not self.ender_info:
            top = 1.1 * cm
        else:
            top = 2.1 * cm
        ...
        if email:
            ...
            txt = ObjectValue(attribute_name='email_padrao.email', display_format='Email: %s',
